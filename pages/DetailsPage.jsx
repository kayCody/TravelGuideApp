...
export default function DetailsPage({route}) {
  ...
  return (
    <View className={``}>
      <Image source={place.image[0]} style={{ height: hp('45%'), width: wp('100%') }} />
      <View className={`p-5 space-y-5`}>
        <View className={`flex-row justify-between items-center`}>
          <View className={``}>
            <Text className={`font-bold text-3xl`}>{place.name}</Text>
            <View className={`flex-row items-center gap-x-1`}>
              <Ionicons name='location-outline' size={15} color='red' />
              <Text className={`text-am text-gray-500`}>{place.location}</Text>
            </View>
          </View>
          <View className={``}>
            ...
            <View className={`flex-row items-center gap-x-1`}>
              ...
              <Text className={`text-sm text-gray-500`}>{place.ratings} (1.7k reviews)</Text>
            </View>
          </View>
        </View>
        <View className={`px-2`}>
