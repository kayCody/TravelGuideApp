...
import { View, Text, SafeAreaView, Image, ImageBackground, TouchableOpacity, Button, Appearance } from 'react-native'
import { widthPercentageToDP as wp, heightPercentageToDP as hp } from 'react-native-responsive-screen';
import { useNavigation } from '@react-navigation/native';
import * as Animatable from 'react-native-animatable';
import travelling from '../assets/images/welcome.avif';
...
export default function WelcomeScreen() {
  ...
  return (
    <View className={`relative h-full  ${theme === 'light' ? 'bg-white' : 'bg-black'}`}>
      <ImageBackground source={travelling} className={`flex-1`} style={{width:wp('100%')}}>
        <View className={` absolute bottom-16  px-6`}>
          {/* Middle section */}
          <View className=''>
            <View className={``}>
              <Text className={`text-2xl ${theme === 'light' ? 'text-white' : 'text-white'}   text-start`}>Welcome to </Text>
          ...
          <View className={`w-full flex items-end justify-center`} style={{ paddingTop: hp('2%') }}>
            <TouchableOpacity className={`rounded-xl  bg-blue-900 items-center justify-center p-3`} style={{ width: wp('40%') }} onPress={() => navigation.navigate("signup")}>
